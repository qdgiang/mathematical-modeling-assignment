...
double MV_CanAir, MV_AirTop, VEC_CanAir, VP_Air_Rate, VP_Top_Rate, MV_TopOut;
double LAI = 2, DeltaH = 0.00000245, Sigma = 65.8, rb = 275, rs = 82, M = 8314;
double VP_Out = 1.3, VP_Can = 3, VP_Air = 12.8, VP_Top = 12;
double M_H2O = 0.018;
double VP_Top_t, VP_Air_t;
...
int main() {
	...
	VEC_CanAir = (2 * p_Air*0.001*LAI) / (DeltaH*Sigma*(rb + rs));

	MV_CanAir = VEC_CanAir * ((VP_Can - VP_Air)/293);
	MV_AirTop = (M_H2O / M)*f_ThScr*(VP_Air - VP_Top);
	MV_TopOut = (M_H2O / M)*f_VentRoof*(VP_Top - VP_Out);

	VP_Air_Rate = (MV_CanAir - MV_AirTop) / cap_CO2Air;
	VP_Top_Rate = (MV_AirTop - MV_TopOut) / cap_CO2Top;

	cout <<"VP AIR RATE: "<< VP_Air_Rate << endl;
	cout <<"VP TOP RATE: " << VP_Top_Rate << endl;
	//Euler 
	VP_Air_t = VP_Air + VP_Air_Rate * h;
	VP_Top_t = VP_Top + VP_Top_Rate * h;
	//Runge-Kutta 4-order
